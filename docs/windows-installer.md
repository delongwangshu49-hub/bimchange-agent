...
The public `v0.2.0-preview.1` remains a historical portable ZIP. v0.5.0 was the first release produced through the per-user installer path described here; v0.8.0-rc.1 is the current prerelease candidate.

公开的 `v0.2.0-preview.1` 作为历史便携 ZIP 保留。v0.5.0 是首个使用本文当前用户安装路径生成的发布版本；v0.8.0-rc.1 是当前预发布候选。
...
从 0.5.0 起，开始菜单与桌面快捷方式显式引用安装目录中的版本化独立 ICO，而不是只依赖 EXE 图标。安装升级时会先移除同名旧快捷方式再重建，以降低 Windows 图标缓存继续显示旧图案的概率。0.8.0-rc.1 的 EXE 与安装器均写入对应 Windows 版本资源。
...
.\scripts\build_windows_installer.ps1 `
  -PortableDirectory .\artifacts\product-dev\BIMChange-Agent-0.8.0-rc.1-win-x64 `
  -OutputRoot .\artifacts\product-dev `
  -PackageVersion 0.8.0-rc.1
```
...
.\scripts\smoke_test_windows_installer.ps1 `
  -InstallerPath .\artifacts\product-dev\BIMChange-Agent-0.8.0-rc.1-win-x64-setup.exe
```
